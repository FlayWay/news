...
#import "LJDisPlayViewController.h"
#import "config.h"
...
@implementation LJDisPlayViewController
...
- (UIButton *)rightBtn {
    ...
    if (!_rightBtn) {
        ...
        [rightBtn setTitle:@"加" forState:UIControlStateNormal];
        [rightBtn setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
        rightBtn.backgroundColor = [UIColor colorWithWhite:1 alpha:0.8];
        _rightBtn = rightBtn;
...
- (void)viewDidLayoutSubviews {
    ...
    // 顶部view的frame
    self.titleScrollView.frame = CGRectMake(0, 0, UIScreen.mainScreen.bounds.size.width - addButtonW, YZTitleScrollViewH);
    
    self.rightBtn.frame = CGRectMake(CGRectGetMaxX(self.titleScrollView.frame), 0, addButtonW, YZTitleScrollViewH);
...
- (void)setLabelTitleCenter:(UILabel *)label {
    ...
    // 计算最大的标题视图滚动区域
    CGFloat maxOffSetX = self.titleScrollView.contentSize.width - UIScreen.mainScreen.bounds.size.width + _titleMargin + addButtonW;
